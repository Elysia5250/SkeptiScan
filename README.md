...
依赖：Python 3.10+, Node.js 18+, 可选 Tesseract(OCR) / Playwright(截图)
...
<details open>
<summary><b>v2.0</b> (2026-07-14) — one-command deploy + self-learning</summary>
<br>

- [x] **一键部署**：`bash start.sh` — 自动装依赖、构建前端、启动服务，单端口 http://localhost:8000
- [x] **后端静态托管前端**：`GET /` 返回前端页面，`GET /api/*` 正常响应
- [x] **自学习系统**：用户通过"纠正"按钮提交意见 → SQLite 存储 → 下次分析自动检索注入 prompt
- [x] **分层 prompt 构建器**：SillyTavern 风格 JSON 分层（system/extra/learning/KB/fact-check/user）
- [x] **纠正弹窗**：分析结果底部"纠正"按钮，选择正确 verdict + 备注
- [x] **纯文本输入**：前端支持直接粘贴商品文案，互斥三种输入
- [x] **状态指示器**：分析中实时显示当前步骤
- [x] **查看测试报告**：前端底部按钮
</details>
